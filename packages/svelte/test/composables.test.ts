...
describe('useBlindspot', () => {
  ...
  it('addEvent delegates to the current route span', () => {
    ...
    addEvent('promo.applied', { code_type: 'percentage' });

    expect(mockAddEvent).toHaveBeenCalledWith('promo.applied', { code_type: 'percentage' });
  });
  ...
  it('startSpan creates a child span, calls fn, then ends the span', () => {
    ...
    const fn = vi.fn();
    startSpan('checkout.address-step', fn);

    expect(mockStartSpan).toHaveBeenCalledWith('checkout.address-step', {}, expect.any(Object));
